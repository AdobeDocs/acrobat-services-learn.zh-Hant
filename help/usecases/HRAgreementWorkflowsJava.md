...
最後，按一下「 *建立認證」* 按鈕。 這會產生認證，範例會自動開始下載。
...
在網頁瀏覽器中渲染後，PDF 會如下所示。 也就是說，您輸入的個人資料會傳播至 PDF：
...
若要將此套件與您的應用程式整合，您必須仿製程式碼。 然後建立 Maven 套件 （mvn 套件），然後將下列檔案安裝到專案中 （您可以在 adobe-sign-sdk 資料夾中的隨附程式碼中找到）：
...
然後，使用 contract.pdf 檔案建立暫時性檔的方法：
