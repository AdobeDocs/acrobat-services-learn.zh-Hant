...
在理想情況下，您不需修改任何合約條款即可接受合約條款。 然而，通常情況下，合約需要自定義，然後需要法律審查。 法律審查會產生重大成本，並降低合約條款的提交流程。 使用根據核准語言變更的預先定義範本，可協助法律團隊管理並更安全地執行合約條款。

此教學課程使用從狀態到狀態不一定的法律合約。 為了解決這些變化，系統會建立具有條件式區段的合約範本，只有在符合特定條件時才包含該區段。 產生的檔可以是 Word 或 PDF 檔。 您也可以瞭解使用 Adobe PDF Services API 或 Acrobat Sign 保護文件的一些方法。
...
在此情況下，系統會傳遞相關信息以協助產生檔，並告知是否應包含特定區段：
...
在數據中，會提供客戶、其名稱、簽署者、其狀態等相關信息。 此外，有關公司正在產生合約的相關信息區段和用於包含合約特定區段的條件旗標。
...
導覽至「 *文件產生標記」* 面板，在檔中放置標籤。

## 插入公司名稱

1. 選取您要取代的文字。 在此情況下，您會在文件的開啟區段中取代 COMPANY。
...
現在，您的標籤已放置在檔案中，您可以預覽產生的合約。

## 預覽產生的檔

您可以直接在 Microsoft Word 內，根據範例 JSON 數據預覽產生的檔。

1. 在「 *文件產生記錄器」*&#x200B;中，選取「 **[!UICONTROL 產生檔」]**。
...
1. 選取「檢視 **[!UICONTROL 檔」]**。
   ...
   ![檢視檔案按鈕的螢幕擷圖](assets/automatelegal_11.png)

1. 瀏覽器視窗隨即開啟，讓您預覽文件結果。

   ![州特定文字的螢幕擷圖](assets/automatelegal_12.png)

## 為每個狀態新增條件條款

在下一個區段中，您僅根據輸入資料條件設定要包含的特定區段。 在範例檔中，第 4 和第 5 節僅與特定狀態相關。 如果是這種情況，只有在客戶處於該狀態時，才應包含特定狀態的條款。 此外，若已移除 Microsoft Word，則不應包含該區段。 使用「檔案產生」API條件式內容功能標記此內容。
...
1. 在 *[!UICONTROL 「選取記錄」]* 字段中，搜尋並選取 **[!UICONTROL customer.state]**。
1. 在「 *[!UICONTROL 選取運算元」]* 字段中，選取 **=**。
1. 在「 *[!UICONTROL 值」]* 欄位中，輸入 *CA*。
1. 選取「 **[!UICONTROL 插入條件」]**。

此區段現在以一些稱為條件式區段標記的標籤的標籤包住。 當您新增標籤時，標記可能已新增條件區段標記為編號行。 您可以在標籤之前回溯移除此項目，否則會將項目編號，就像產生檔時標籤不存在一樣。 以條件為條件的區段結尾為 `{% end-section %}` 標籤。
...
產生檔時，請注意，所包含的區段僅符合數據標準。 在下列範例中，因為該州等於CA，因此只包含加州區段。

![加州揭露信息螢幕擷圖](assets/automatelegal_18.png)

另一個顯著的變更是後續區段「使用服務」和「軟體」的編號為 5。 這表示在省略「華盛頓」區段時，頁碼會繼續。

![持續編號的螢幕擷圖](assets/automatelegal_19.png)

若要測試客戶在華盛頓特區 （而不是加州） 時範本是否正確運作，請變更範本的範例數據：

1. 在 Document *Generation Tagger 中*，選取「 **[!UICONTROL 編輯輸入數據」]**。

   ![檔世代 Tagger 螢幕擷圖](assets/automatelegal_20.png)

1. 選取「 **[!UICONTROL 編輯」]**。

1. 在 JSON 數據中，將 CA *變更*&#x200B;為 *WA*。

   ![JSON 數據螢幕擷圖](assets/automatelegal_21.png)
...
![僅包含「浪費」狀態區段的文件螢幕擷圖](assets/automatelegal_22.png)
...
1. 在「 *[!UICONTROL 文件產生記錄器」]*&#x200B;中，選取「進 **[!UICONTROL 階」]**。
1. 擴充 **[!UICONTROL 條件式內容]**。
1. 在「內容」類型下&#x200B;*[!UICONTROL 方，選取&#x200B;**[!UICONTROL 「詞組」]**。]*
1. 在 *[!UICONTROL 「選取記錄」]* 字段中，搜尋並選取 **[!UICONTROL customer.state]**。
1. 在「 *[!UICONTROL 選取運算元」]* 字段中，選取 **=**。
1. 在「 *[!UICONTROL 值」]* 欄位中，輸入 *CA*。
1. 選取「 **[!UICONTROL 插入條件」]**。

雖然標籤的名稱相同，但「片語」和「區段」的主要不同之處在於，片語有不包含新行的區段。 條件區段標籤和 -end-section 標籤必須位在同一段落中。
...
1. 在 *[!UICONTROL 「文件產生記錄器」]*&#x200B;中，選 **[!UICONTROL 取「Adobe Sign]**」。
1. 在 *[!UICONTROL 「指定收件者數目]* 」字段中，設定收件者人數 （此範例使用 2）。
...
接下來，為簽署者放置數據欄位，該簽署者在簽署時會自動填入該字段。
...
您現在已為檔加上標籤，準備就緒。 在下一節中，瞭解如何使用「文件產生」API為Node.js產生檔範例。 這些範例適用於任何語言。

在註冊認證時，開啟您下載的 pdfservices-node-sdk-samples-master 檔案。 這些檔案包括pdfservices-api-credentials.json和private.key檔案。

1. 開啟終端 **[!UICONTROL 機]** 以使用 `npm install`.
1. 將範例 *data.json* 複製到 *資源檔* 夾中。
1. 複製您在資源檔案&#x200B;*夾中建立的* Word 範本。
...
您也必須分別為 PDF 或 DOCX 輸出格式，將輸出檔案的名稱變更為.pdf或.docx：
...
1. 在「 *收件者」* 欄位中，指定兩個電子郵件位址。 最好使用與您的 Acrobat Sign 帳戶無關的電子郵件位址。
...
1. 選取「**[!UICONTROL 下一步]**」。
1. 向下捲動至簽名頁面時，您可以根據標籤查看已放置的簽名字段。

   ![簽名字段的螢幕擷圖](assets/automatelegal_30.png)
...
1. 選 **[!UICONTROL 取「按下這裡」以簽署]**。

   ![按兩下這裡進行簽署的螢幕擷圖](assets/automatelegal_33.png)
...
1. 選取 **[!UICONTROL 「授權」]**。
1. 系統會透過彈出視窗提示您使用Acrobat Sign 帳戶登入。 登入管理員的使用者名稱和密碼。
...
由於已從先前步驟新增授權令牌，您必須上傳檔才能進行API呼叫：
...
**POST合約API** 傳回合約的ID。 若要取得 JSON 模型結構的範本，請選取「 **最小模型結構」**。 您可以在「完成模型結構&#x200B;**」區段中**&#x200B;找到完整的參數清單。
...
1. 將上一個API呼叫回應的 agreementId 複製到 agreementId 字段。
1. 選取 **[!UICONTROL 「試用！」]**。
...
然後，您可以在「 **回應內**&#x200B;文」中，將其下載到您的計算機，並儲存在您喜歡的位置。
