...
本實作教學課程說明如何為貴公司建立專門的 Microsoft Word NDA 範本。 Adobe適用於 Microsoft Word Adobe [Document Generation Tagger](https://opensource.adobe.com/pdftools-sdk-docs/docgen/latest/wordaddin.html#add-in-demo) 的免費載入宏會插入「卷標」以輸入動態值。 瞭解如何將 JSON 數據傳遞至範本並建立動態 PDF。 產生的 PDF 可以透過電子郵件傳送給您的共同作業人員，或是根據您的業務需求和目標，在共同作業人員的瀏覽器中顯示。 若要跟進客戶，您只需要體驗Node.js、JavaScript、Express.js、HTML 和 CSS。

## 相關 API 和資源

藉助 [!DNL Adobe Acrobat Services]此功能，您可以使用動態數據，隨時隨地產生 PDF 檔。 [!DNL Acrobat Services] 提供一套 PDF 工具，包括可自動 [建立 NDA 的Adobe文件產生](https://www.adobe.io/apis/documentcloud/dcsdk/nda-creation.html)API。
...
您可以將免費的 [Document Generation Tagger 載入宏](https://opensource.adobe.com/pdftools-sdk-docs/docgen/latest/wordaddin.html#add-in-demo) 安裝至 Microsoft Word。 如果您屬於組織，您可以要求您的 Microsoft Office 管理員為所有人安裝免費載入宏。

安裝載入宏後，您可以在「首頁」索引標籤下方的「Adobe」類別中找到載入宏。 若要開啟索引標籤，請選取&#x200B;****「產生檔」：

![Word 中「文件產生」載入宏的螢幕擷圖](assets/nda_1.png)

您可以在索引標籤內上傳範例 JSON 檔。 本檔可以是範例，因為您僅使用此範例來建立 Microsoft Word 範本。

![「檔產生」載集中的樣本資料螢幕擷圖](assets/nda_2.png)

選 **取「產生標籤** 」以檢視您可以在範本中使用的專案。 以下是從 JSON 結構擷取的屬性，可隨時在範本中使用：

![「檔產生」載集中文字標籤的螢幕擷圖](assets/nda_3.png)

這些是欄位中的 `authorizedSigner` 功能。 其他欄位會隨即包住，您可以在 Microsoft Word 中展開檢視。 載入宏也提供進階數據選項，例如表格、清單、計算值等。

## 建立標籤

隨意建立範本或將現有範本](https://www.adobe.io/apis/documentcloud/dcsdk/doc-generation.html#sample-blade)匯[入 Microsoft Word。在您設定檔后，按兩下載入宏中的對應字元，即可將標籤新增至每個欄位。
...
此程式代碼在呼叫 SDK API中使用 JSON 檔和 Microsoft [!DNL Adobe Acrobat Services] Word 範本。 回應時，您會收到輸出，並儲存至應用程式的檔案系統。 您可以透過電子郵件將產生的文件轉寄給客戶，或使用免費 [的Adobe PDF內嵌API](https://www.adobe.io/apis/documentcloud/dcsdk/pdf-embed.html)在瀏覽器中向客戶顯示預覽。
...
現在使用 [Adobe Sign API](https://www.adobe.io/apis/documentcloud/sign.html) 在 NDA 上請求簽名，並將最終的已簽署檔傳送給所有當事人。 Adobe Sign使用 Webhook](https://www.adobe.io/apis/documentcloud/sign/docs.html#!adobedocs/adobe-sign/master/webhooks.md) 通知您[。聽此 Webhook 的話，您可以摻取 NDA 的狀態。

如需對Adobe Sign程式的更深入說明， [請參閱檔](https://www.adobe.io/apis/documentcloud/sign/docs.html) 或閱讀此深入部落格文章。

## 後續步驟

在此實作教學課程中，使用 Adobe Document Generation Tagger 以使用 Microsoft Word 範本和 JSON 數據檔動態產生 PDF 檔。 載入宏協助 [自動建立為每一方自定義的 NDA](https://www.adobe.io/apis/documentcloud/dcsdk/nda-creation.html) ，然後使用 Sign 收集簽名API。

您可以使用這些技術動態建立您自己的 NDA 或其他檔，讓團隊有時間專注於高效率的工作。 探索 [[!DNL Adobe Acrobat Services]](https://www.adobe.io/apis/documentcloud/dcsdk/pdf-tools.html) 尋找適合您語言和所選運行時間的 API 和 SDK，以便您可以將 PDF 功能直接新增到應用程式，以快速建立 PDF 檔。 [立即開始](https://www.adobe.io/apis/documentcloud/dcsdk/gettingstarted.html)六個月免費試用
