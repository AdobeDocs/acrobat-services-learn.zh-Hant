...
數位化帶來挑戰。 現在，大多陣列織有許多類型的 [ 法律合約 ](https://www.adobe.io/apis/documentcloud/dcsdk/legal-contracts.html) ，他們必須建立、編輯、核准以及由不同方簽署。 這些法律合約通常需要獨一無二的自訂和品牌化。 組織在完成簽署後，也可能需要將它們儲存為受保護的格式，以確保安全。 要完成所有這些工作，他們需要強大的檔產生和管理解決方案。
...
若要進一步說明，假設合約中有一個您想要包含的區段，只有條件式。 在「選取內容類型」欄位中，選取「 **區段」。** 在「選取記錄」欄位中，選擇決定是否顯示條件式區段的選項。 選取您需要的條件式運算子，然後設定您在「值」欄位中測試的值。 然後按一下「 **插入條件」。** 下方的螢幕擷取會說明此過程。
...
為確保資料完整性，請以受保護的格式儲存法律檔。 透過 [!DNL Acrobat Services] API，您可以快速地將檔轉換為 PDF 格式。 您可以建立簡單的 express Node.js 應用程式，將檔產生API整合，並使用這個簡單的應用程式將標籤檔從 Word 轉換為 PDF 格式。
...
若要採用可用支援的檔案格式來建立 PDF 檔，請使用 . [!DNL Acrobat Services]
